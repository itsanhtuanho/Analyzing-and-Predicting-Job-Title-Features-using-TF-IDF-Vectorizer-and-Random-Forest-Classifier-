...
# Modeling
ab = AdaBoostClassifier(n_estimators=100, random_state=69)
scoring = {
    'accuracy': 'accuracy',
...
ee_ab_func = EasyEnsembleClassifier(base_estimator=ab, random_state=69, verbose=2)
    # Cross-validation
cv_scores_func_ee = cross_validate(ee_ab_func, X_train, y1_train, cv=5, scoring=scoring)
...
ee_ab_function_classifier = ee_ab_func.fit(X_train, y1_train)
    # Prediction
ypred_test_func_ee = ee_ab_function_classifier.predict(X_test)
...
## Role
ee_ab_role = EasyEnsembleClassifier(base_estimator=ab, random_state=69, verbose=2)
    # Cross-validation
cv_scores_role_ee = cross_validate(ee_ab_role, X_train, y2_train, cv=5, scoring=scoring)
    # Getting averaged metrics from cross-validation
...
mean_f1_role_ee = cv_scores_role_ee['test_f1'].mean()
    # Fit on training data
ee_ab_role_classifier = ee_ab_role.fit(X_train, y2_train)
    # Prediction
ypred_test_role_ee = ee_ab_role_classifier.predict(X_test)
    # Metric on Testing data
...
## Level
ee_ab_lvl = EasyEnsembleClassifier(base_estimator=ab, random_state=69, verbose=2)
    # Cross-validation
cv_scores_lvl_ee = cross_validate(ee_ab_lvl, X_train, y3_train, cv=5, scoring=scoring)
    # Getting averaged metrics from cross-validation
...
mean_f1_lvl_ee = cv_scores_lvl_ee['test_f1'].mean()
    # Fit on training data
ee_ab_lvl_classifier = ee_ab_lvl.fit(X_train, y3_train)
    # Prediction
ypred_test_lvl_ee = ee_ab_lvl_classifier.predict(X_test)
    # Metric on Testing data
...
probabilities_func_ee = ee_ab_function_classifier.predict_proba(X_test)
probabilities_role_ee = ee_ab_role_classifier.predict_proba(X_test)
probabilities_lvl_ee = ee_ab_lvl_classifier.predict_proba(X_test)

pred_conf_func = prediction_confidence(probabilities_func_ee)
anti_class_func = anti_class(ee_ab_function_classifier, probabilities_func_ee)

pred_conf_role = prediction_confidence(probabilities_role_ee)
anti_class_role = anti_class(ee_ab_role_classifier, probabilities_role_ee)

pred_conf_lvl = prediction_confidence(probabilities_lvl_ee)
anti_class_lvl = anti_class(ee_ab_lvl_classifier, probabilities_lvl_ee)
